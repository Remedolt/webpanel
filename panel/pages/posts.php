...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_post') {
    ...
    if ($deleteId > 0) {
        $gone = cms_row_title($pdo, 'posts', $deleteId);
        $stmt = $pdo->prepare('DELETE FROM posts WHERE id = ?');
        $stmt->execute([$deleteId]);
        flash_set('success', 'Yazı silindi.');
        cms_audit($pdo, 'delete', 'post', $deleteId, $gone);
    }
                ...
                <?php if (!$posts): ?>
                    <tr>
                        <td colspan="7" class="px-4 py-12 text-center">
                            <p class="text-slate-500">Kayıt bulunamadı.</p>
                            <?php if ($q === '' && $statusFilter === ''): ?>
                                <a class="mt-2 inline-block text-sm font-semibold text-[#2271b1] hover:underline" href="index.php?page=post-new">İlk yazıyı yaz</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php else: ?>
