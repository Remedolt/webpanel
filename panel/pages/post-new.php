...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    cms_audit($pdo, $editId > 0 ? ($status === 'publish' ? 'publish' : 'update') : ($status === 'publish' ? 'publish' : 'create'), 'post', $postId, $title);
...
<form method="post" action="index.php?page=post-new<?= $isEdit ? '&amp;id=' . (int) $post['id'] : '' ?>" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
    <?= csrf_field() ?>
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
            ...
            <textarea id="content" name="content" rows="18" placeholder="Yazınızı buraya yazın…"
                      class="cms-editor w-full resize-y rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"><?= e((string) $post['content']) ?></textarea>
        </div>
