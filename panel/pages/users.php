    ...
    if ($action === 'create') {
        ...
        if ($username === '' || $email === '' || $display === '' || strlen($password) < 8) {
            ...
            try {
                ...
                cms_audit($pdo, 'create', 'user', (int) $pdo->lastInsertId(), $display);
...
try {
    $users = $pdo->query('SELECT id, username, email, display_name, role, avatar, created_at FROM users ORDER BY id ASC')->fetchAll();
} catch (PDOException $e) {
...
<div class="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
    <div>
        ...
        <p class="mt-1 text-sm text-slate-500">Panel erişimi olan hesaplar. Avatar için sağ üst menüden <a class="text-[#2271b1] hover:underline" href="index.php?page=profile">Profil</a>.</p>
    </div>
</div>
