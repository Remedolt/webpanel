    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        wp: { accent: '#2271b1', sidebar: '#0f172a' }
                    }
                }
            }
        }
...
<div id="admin-shell" class="min-h-screen lg:grid lg:grid-cols-[15rem_1fr] lg:grid-rows-[3.5rem_1fr]">
        ...
        <form action="index.php" method="get" class="flex-1 max-w-md">
            ...
            <label class="relative block">
                ...
                <input type="search" name="q" value="<?= e($searchQuery) ?>" placeholder="Yazılarda ara…"
                       class="w-full rounded-md border border-slate-200 bg-slate-50 py-1.5 pl-9 pr-3 text-sm text-slate-800 placeholder:text-slate-400 focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-200">
            </label>
        </form>
        ...
        <div class="ml-auto flex items-center gap-1">
            <div class="relative" data-dropdown>
                <button type="button" class="relative rounded-md p-2 text-slate-600 hover:bg-slate-100" data-dropdown-button aria-label="Bildirimler">
                    ...
                    <?php if ($pendingComments > 0 || $unreadMessages > 0): ?>
                ...
                <div class="hidden absolute right-0 mt-2 w-80 rounded-lg border border-slate-200 bg-white py-2 shadow-lg z-40" data-dropdown-panel>
                    ...
                    <a href="index.php?page=comments" class="block px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">
                        <?= $pendingComments > 0 ? e((string) $pendingComments . ' yorum onay bekliyor.') : 'Onay bekleyen yorum yok.' ?>
                    </a>
                    <a href="index.php?page=messages" class="block px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">
                        <?= $unreadMessages > 0 ? e((string) $unreadMessages . ' yeni iletişim mesajı.') : 'Yeni iletişim mesajı yok.' ?>
                    </a>
                </div>
            </div>
            ...
            <div class="relative" data-dropdown>
                <button type="button" class="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-slate-700 hover:bg-slate-100" data-dropdown-button>
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-xs font-semibold text-white">
                        <?= e(function_exists('mb_substr') ? mb_strtoupper(mb_substr((string) $currentUser['display_name'], 0, 1)) : strtoupper(substr((string) $currentUser['display_name'], 0, 1))) ?>
                    </span>
                ...
                <div class="hidden absolute right-0 mt-2 w-52 rounded-lg border border-slate-200 bg-white py-1 shadow-lg z-40" data-dropdown-panel>
                    <a href="index.php?page=users" class="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
